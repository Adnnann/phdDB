    ...
    if (users?.data) {
      _.chain(Object.values(users.data))
        ...
        .map((item, index) => {
          const firstCol = (
            <div>
              {item.authors.replace(/[^\x00-\x7F]/g, "")}
              <br />
          ...
          const secondCol = (
            <div>{item.title.replace(/[^\x00-\x7F]/g, "")}</div>
          );
          const thirdCol = <div>{item.year}</div>;

          const fourthCol = <div>{item.mainArea}</div>;

          const fifthCol = (
            <div>
              {item.subject
                ? item.subject.replace(/[^\x00-\x7F]/g, "")
                : item.subject}
            </div>
          );

          const eleventhCol = <div>{item.relevance}</div>;
          const twelwethCol = (
            <>
  ...
  const handleChange = (name) => (event) => {
    ...
    if (name === "accountStatus") {
      ...
      const users = {
        ...
        lastItem: 10,
        role:
          filters.filterStatus === "isInformative"
            ? "informative"
            : filters.filterStatus === "isAdmin"
            ? "admin"
            : filters.filterStatus === "isMentor"
